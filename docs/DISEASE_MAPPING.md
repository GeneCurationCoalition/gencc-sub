...
`DiseaseResolver` trusts whatever is stored under these keys, so the rows the previous importer wrote have to be rewritten when this policy is deployed. A plain `update:diseases` run would not do that until upstream next published, because it skips any source whose file headers match the ones it last recorded. The migration `expire_disease_source_file_headers` clears the legacy mapping payloads on MONDO and Orphanet rows, then deletes those sources' recorded headers. The next `update:diseases` run therefore treats both sources as changed and repopulates current exact mappings. Rows are retained because submissions reference their stable IDs. OMIM rows did not change shape, so their data and headers are kept.

Every Ansible deploy invokes `update:diseases --no-interaction` after migrations and waits for it. The command's source-header cache makes unchanged sources a no-op. Restore deployments already use maintenance mode; for the one-time production `migrate_only` transition, maintenance mode is supplied as a local deployment variable so the checked-in inventories remain general-purpose. A failed transition therefore leaves the portal down instead of exposing empty or partially rebuilt mappings. Normal scheduled updates continue through the disease-update timer.

Rows whose terms have left their source entirely remain in the database for historical foreign keys, but their pre-policy mapping payload was cleared by the transition migration and is not restored by reconciliation.
