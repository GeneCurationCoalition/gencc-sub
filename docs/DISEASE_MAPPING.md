...
For the 2.2.0 staging rollout, the inventory enables a deploy-time `update:diseases --no-interaction` run after migrations. The playbook keeps the portal in maintenance mode, waits for the command to finish, and fails the deployment if any source phase fails. This rewrites the expired MONDO and Orphanet rows before the portal resumes; normal scheduled updates continue through the disease-update timer.
