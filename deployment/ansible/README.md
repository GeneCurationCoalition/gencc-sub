...
It is intentionally written so it can be run safely after Terraform provisions the VM (no public IP; SSH via IAP).

## Layout
- `playbooks/site.yml` — main entrypoint
- `inventories/production.ini.example` — example inventory (Terraform also outputs one)
- `group_vars/` — non-secret defaults + a vault template
...
2. Create `group_vars/all.vault.yml` from the template and encrypt it:
